...
public class SuppliesFunctional {
    ...
    public static void loadMenu(){
        ...
        switch(op){
            ...
            case "7":
                ...
                getVentasMujerTiendaInstore();
                break;

            case "8":
                System.out.print("Agrupación de productos por etiquetas(tags)");

                break;

            case "9":
                System.out.print("Cuantos hombres y mujeres usaron cupón ");
                getHombresUsaronCuponMujeresUsaronCupon();
                break;

            case "10":
                System.out.print("Obtenga la venta con la compra más costosa y la venta con la compra más barata");
                getVentaMayorMenorCosto();
                break;
    ...
    /**Calcule el monto total que pagó el cliente en cada venta.*/
    public static ArrayList<Sale> TotalVentas(){
   ...
   /* 7. Obtenga todas las ventas en las que el comprador es una mujer y fue comprado en la tienda ('in store') */
    ...
    public static void getVentasMujerTiendaInstore(){
        Predicate<Sale> ventas = sale -> sale.getCustomer().getGender().equals("F") && sale.getPurchasedMethod().equals("In store");
        ArrayList<Sale> result = sales.stream().filter(ventas).collect(Collectors.toCollection(ArrayList::new));
        result.forEach(System.out::println);
    }
  ...
  /*  9. Obtenga cuantos hombres usaron cupón y cuantas mujeres usaron cupón */
    ...
    public static void getHombresUsaronCuponMujeresUsaronCupon(){
        Predicate<Sale> cuponHombres = sale -> sale.getCouponUsed().equals(true) && sale.getCustomer().getGender().equals("M");
        Predicate<Sale> cuponMujeres = sale -> sale.getCouponUsed().equals(true) && sale.getCustomer().getGender().equals("F");
        long thombres= sales.stream().filter(cuponHombres).count();
        long tmujeres= sales.stream().filter(cuponMujeres).count();
        System.out.println("Total cupon Hombres "+thombres);
        System.out.println("Total cupon Mujeres "+tmujeres);
  ...
  /*  10. Venta con mayor costo y menor costo" */
    public static void getVentaMayorMenorCosto(){
        ...
        DoubleSummaryStatistics collect = result.stream().collect(Collectors.summarizingDouble(Sale::getTotal));

        System.out.println("compra mas costosa: " + collect.getMax());
        System.out.println("compra menos costosa: " + collect.getMin());
